...
export interface RunTurnOptions<C> {
    ...
    /** Stream the response when the provider supports it. */
    stream?: boolean;
    /** Called with each text delta while streaming. */
    onToken?: (delta: string) => void;
    /** Called with each reasoning delta while streaming. */
...
export async function runTurn<C>(opts: RunTurnOptions<C>): Promise<TurnResult> {
    ...
    // Use streaming when requested and supported; otherwise a single response.
    const useStream = opts.stream === true && typeof provider.chatStream === "function";
    const getResponse = () => {
        if (useStream) {
            return provider.chatStream!({ messages, tools }, (event) => {
                if (event.type === "text") opts.onToken?.(event.delta);
                else if (event.type === "thinking") opts.onThinking?.(event.delta);
            });
        }
